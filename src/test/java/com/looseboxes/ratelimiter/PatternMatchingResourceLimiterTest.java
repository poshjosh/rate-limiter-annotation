...
class PatternMatchingResourceLimiterTest {
    ...
    private ResourceLimiter<Object> buildRateLimiter(int expectedNodes, Class<?>... classes) {
        ...
        Node<NodeValue<ResourceLimiter<Object>>> rateLimiterRootNode = ResourceLimiterFromAnnotationFactory.of().createNode(classes);
        ...
        PatternMatchingResourceLimiter.MatcherProvider<Object> matcherProvider = (nodeName, nodeValue) -> {
            return key -> nodeName;
        };

        boolean firstMatchOnly = false; // false for annotations, true for properties
        return new PatternMatchingResourceLimiter<>(matcherProvider, (Node)rateLimiterRootNode, firstMatchOnly);
    }
